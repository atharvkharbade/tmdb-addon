...
async function getFavorites(type, language, page, genre, config) {
    ...
    return fetchFunction(parameters)
        .then((res) => ({
            metas: sortResults(res.results, genre).map(el => parseMedia(el, type, genreList, config.rpdbkey, config.rpdbMediaTypes, config.topposterskey, config.toppostersConfig, language))
        }))
        .catch(console.error);
}
